...
Just to illustrate some of these functions (or "verbs"), we will use the `diamonds` dataset from `tidyverse`, which contains the details and prices of over 50,000 diamonds.
```{r}
df <- diamonds
head(df)
```
...
```{r}
filter(df, cut == "Premium")
```

We can construct new variables using the `mutate` function, perhaps to compute a derived value.
For example, we might want to split up the diamonds into weight classes.
```{r}
mutate(df, weight_class = cut(carat, c(0, 1, 2, 3, 4, 5)))
```
...
For example, we can group the diamonds by their cut, and find the mean price for each group, as well as the number of diamonds in each group.
```{r}
df |> 
  group_by(cut) |> 
...
For example, we may want to show the relationship between weight (in carat) and price, plotting both individual points (for each diamond) as well as a trendline.
```{r}
